...
impl StringSupervisor {
    ...
    pub fn is_duplicate_mask(&self) -> Vec<bool> {
        let is_duplicate = self.hash_vec.iter().map(|x| self.duplicate_hash_set.contains(&x)).collect();
        is_duplicate
    }
    ...
    pub fn filter_range(&self) -> Vec<Range<usize>>{
        ...
        for (index, window) in duplicate_mask.windows(2).enumerate() {
            ...
            match (*i0, *i1) {
                ...
                // End of window:
                (true, false) => {
                    filter_range_vec.push(range_start..range_end)
                },
                // Start of window:
                (false, true) => {
                    range_start = index;
